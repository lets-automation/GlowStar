...
def _market_month_index(df: pd.DataFrame) -> pd.Series:
    """Months since the earliest MarketSheetDate — the pricing-time clock.
...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    x["market_month_index"] = _market_month_index(df)

    # Categorical (pandas 'category' dtype -> native GBM handling, robust to
    # unseen/rare levels).
    for col in CATEGORICAL_FEATURES:
        x[col] = df[col].astype("string").fillna("NA").astype("category")
...
def _assert_no_leakage(x: pd.DataFrame) -> None:
    """Raise if any transaction-derived column leaked into the feature matrix."""
